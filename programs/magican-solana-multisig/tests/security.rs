//! Главная витрина проекта: по одному негативному тесту на каждый вектор атаки
//! из модели угроз (раздел 5 PRD). Каждый тест ожидает, что атака проваливается.
...
/// Создаёт мультисиг с owner1 + случайным вторым владельцем и заданным порогом.
fn make_multisig(
...
// #1 — Не-владелец не может предлагать/одобрять.
#[test]
fn vector_01_non_owner_cannot_propose_or_approve() {
    ...
    assert_err_log(res, "NotAnOwner"); // #1: не-владелец не может предлагать

    // валидное предложение от owner1, затем approve от чужака
    let (ta, data, _) = sol_transfer_parts(&signer, &Pubkey::new_unique(), SOL);
    ...
    assert_err_log(res, "NotAnOwner"); // #1: не-владелец не может одобрять
}

// #2 — Исполнение при одобрениях < threshold.
#[test]
...
// #4 — Двойное одобрение одним владельцем не накручивает счётчик.
#[test]
fn vector_04_double_approve_is_not_counted_twice() {
    ...
    // owner1 голосует повторно — маска остаётся [true,false,false], одобрений всё ещё 1.
...
// #5 — Уволенный владелец доисполняет старое предложение после смены владельцев.
#[test]
fn vector_05_stale_owner_set_cannot_execute() {
    let (mut svm, pid) = setup();
    // owners [o1, o2], threshold 1 (чтобы предложения были готовы после автоголоса).
    ...
    // Предложение A (index 0) — готово, но НЕ исполняем.
    ...
    // Предложение B (index 1) — set_owners([o1]) → seqno 0→1. Исполняем.
...
fn vector_06_direct_change_threshold_fails() {
    ...
    assert!(failed, "#6: прямой change_threshold без self-CPI должен падать");
}

// #7 — Reinit существующего мультисига.
#[test]
fn vector_07_reinit_fails() {
    ...
    // Повторный create с тем же creator+seed → тот же PDA → init падает.
    let res = create_multisig(&mut svm, &pid, &owner1, &owners, 1, 7);
    assert!(res.is_err(), "#7: reinit существующего мультисига должен падать");
}

// #8 — Подмена PDA не-каноническим/чужим адресом.
#[test]
fn vector_08_wrong_pda_address_fails() {
    ...
    // Аккаунт выведен для seed=100, но в инструкции seed=8 → seeds-констрейнт не сойдётся.
...
// #9 — Некорректный threshold через governance (0).
#[test]
fn vector_09_change_threshold_zero_fails() {
    ...
    let tx = transaction_pda(&pid, &ms, 0);
    // Внутренний change_threshold(0) падает → весь execute реверте.
    let res = execute_transaction(&mut svm, &pid, &ms, &tx, &owner1, remaining);
    assert_err_log(res, "InvalidThreshold"); // #9
    assert!(!fetch_transaction(&svm, &tx).did_execute, "did_execute не выставлен при реверте");
}

// #10 — Дубликаты владельцев через governance.
#[test]
...
// #11 — Эскалация привилегий при CPI: программа не может подписать за чужой аккаунт.
#[test]
fn vector_11_cpi_privilege_escalation_fails() {
    let (mut svm, pid) = setup();
    let (owner1, _owners, ms, _signer) = make_multisig(&mut svm, &pid, 1, 2, 11);

    // Жертва — обычный System-аккаунт со средствами, который мы НЕ контролируем.
    let victim = Pubkey::new_unique();
    svm.airdrop(&victim, 3 * SOL).unwrap();
    let recipient = Pubkey::new_unique();

    // Предложение: перевод SOL ИЗ victim (victim помечен is_signer=true).
    ...
    // invoke_signed подписывает только за treasury-PDA, но не за victim → CPI падает.
    let res = execute_transaction(&mut svm, &pid, &ms, &tx, &owner1, remaining);
    assert!(res.is_err(), "#11: программа не может подписать за чужой аккаунт");
    assert_eq!(svm.get_balance(&victim), Some(3 * SOL), "средства жертвы не тронуты");
}
